...
beforeAll(() => {
    token = jsonwebtoken_1.default.sign({ userId }, process.env.JWT_SECRET || 'fallback_secret', { expiresIn: '1h' });
});
beforeEach(() => {
    jest.clearAllMocks();
    supabaseClient_1.supabase.from.mockReturnValue(mockBuilder);
});
describe('Vault API', () => {
    describe('GET /api/vault', () => {
        it('should create new sync state if none exists', async () => {
            ...
            mockBuilder.then.mockImplementationOnce((cb) => cb({ data: [] }));
