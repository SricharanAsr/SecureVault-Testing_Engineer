...
router.get('/', auth_1.authMiddleware, async (req, res) => {
    try {
        ...
        let { data: syncState } = await supabaseClient_1.supabase
            .from('user_sync_state')
            .select('*')
            .eq('user_id', userId)
            .maybeSingle();
        if (!syncState) {
            const { data: newState, error } = await supabaseClient_1.supabase
                .from('user_sync_state')
        ...
        const { data: entries } = await supabaseClient_1.supabase
            .from('vault_entries')
            .select(`
...
router.post('/sync', auth_1.authMiddleware, async (req, res) => {
    try {
        ...
        const { data: syncState } = await supabaseClient_1.supabase
            .from('user_sync_state')
            .select('*')
            .eq('user_id', userId)
            .maybeSingle();
        if (!syncState)
            return res.status(404).json({ error: 'Vault sync state not found' });
        ...
        const { data: finalEntries } = await supabaseClient_1.supabase
            .from('vault_entries')
            .select('*')
            .eq('user_id', userId);
